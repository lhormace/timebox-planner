...
export function TaskPlacementDialog({ task, onClose, onEdit }: Props) {
  ...
  const [newMemberId, setNewMemberId] = useState(assignedMembers[0]?.id ?? "");
  const [newDate, setNewDate] = useState("");
  const [newHours, setNewHours] = useState(8);

  const project = projects.find((p) => p.id === task.projectId);

  const totalPlaced = getPlacedHours(task);
  ...
  const handleAdd = () => {
    ...
    addPlacement(task.id, { memberId: newMemberId, date: newDate, hours: Math.min(newHours, 8) });
    setNewDate("");
    setNewHours(8);
  };

  const sortedPlacements = [...task.placements].sort(
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="space-y-1 max-h-48 overflow-y-auto">
          ...
          {sortedPlacements.map((p) => {
            const placementMember = members.find((m) => m.id === p.memberId);
            return (
              <div
                key={`${p.memberId}-${p.date}`}
        ...
        <div className="border-t pt-3 space-y-2">
          ...
          {assignedMembers.length === 0 ? (
            <p className="text-xs text-gray-400">
              担当者が未定のため配置できません。先に「編集」から担当者を割り当ててください。
            </p>
          ) : (
            <div className="flex gap-2 items-end">
              <div className="grid gap-1">
                ...
                <Select
                  items={memberItems}
                  value={newMemberId}
                  onValueChange={(v) => v && setNewMemberId(v)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
